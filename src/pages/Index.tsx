...
type BlockType = "text" | "social" | "track" | "banner"
...
interface CardStyle {
  bg: string          // фон плашки (hex)
  border: string      // цвет обводки
  hoverBorder: string // цвет обводки при hover
  textColor: string   // цвет текста
  linkColor: string   // цвет ссылок/кнопок
}
  ...
  cards: [
    {
      ...
      blocks: [
        { id: "b1", type: "banner", bannerText: "✨ Добро пожаловать на мою платформу!", bannerImage: "" },
      ],
    },
    {
      id: 2, size: "large", style: { ...DEFAULT_STYLE },
      blocks: [
        { id: "b2", type: "social", title: "Мои платформы", links: [] },
      ],
    },
    {
      id: 3, size: "medium", style: { ...DEFAULT_STYLE },
      blocks: [
        { id: "b3", type: "text", title: "Обо мне", emoji: "🚀", description: "Расскажите о себе здесь." },
      ],
    },
    {
      id: 4, size: "large", style: { ...DEFAULT_STYLE },
      blocks: [
        { id: "b4", type: "track", trackTitle: "Мой трек", trackUrl: "" },
      ],
    },
  ],
}
...
function saveData(d: SiteData) { localStorage.setItem("nebula_data_v3", JSON.stringify(d)) }
...
function BlockView({
  ...
  const links: PlatformLink[] = block.links ?? []
    ...
    <div className="relative">
      ...
      {block.type === "banner" && (
        <div
          ...
          style={block.bannerImage ? { backgroundImage: `url(${block.bannerImage})`, backgroundSize: "cover", backgroundPosition: "center" } : { background: `linear-gradient(135deg, ${cardStyle.linkColor}22, ${cardStyle.bg})` }}
        >
          ...
          <div className="flex flex-wrap gap-2">
            {PLATFORMS.map(pl => {
              ...
              return active && !isAdmin ? (
                <a key={pl.id} href={url} target="_blank" rel="noopener noreferrer"><div className="w-10 h-10 rounded-xl flex items-center justify-center transition-all hover:scale-110" style={{ backgroundColor: pl.bg, color: pl.color }}><PlatformSvg id={pl.id} size={18} /></div></a>
              ) : inner
            })}
          </div>
      ...
      {isAdmin && (
        <div className="absolute -top-3 right-2 flex gap-1 z-20 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          {(["small", "medium", "large"] as const).map(sz => (
            <button
              key={sz}
              onClick={() => onUpdate({ ...card, size: sz })}
              className={`px-2 py-0.5 rounded-md text-xs border transition-all ${card.size === sz ? "bg-purple-500/40 border-purple-400/60 text-purple-200" : "bg-black/60 border-white/15 text-white/40 hover:text-white/70"}`}
            >
              {sizeMap[sz].label[0]}
            </button>
          ))}
